//Xander Nuttle
//call_pb_cn_v2.c
//Call: ./call_pb_cn_v2 guidecounts_file
//
//Takes a file containing counts of molecular tags corresponding to U6 MIP capture events for each guide RNA, with the first count
//corresponding to the original U6 MIP and the second count corresponding to the new U6 MIP (which captures the guide and its molecular tag)
//and prints to standard output the sample name followed by the number of piggyBac (PB) insertions for that sample, inferred from the U6 MIP
...
int main(int argc,char*argv[])
{
	...
	//determine number of guides to be analyzed
  ...
  count_guides(guidecounts,&numguides);
	...
	{
		fscanf(guidecounts,"%*s %ld %ld",&c1,&c2);
		counts[g]=c1+c2;
	}
